    ...
    public class HoaDonBanController : ControllerBase
    {
        ...
        public IActionResult Search([FromBody] Dictionary<string, object> formData)
        {
            try
            {
                ...
                string ma_NV = "";
                if (formData.Keys.Contains("ma_nhan_vien") && !string.IsNullOrEmpty(Convert.ToString(formData["ma_nhan_vien"]))) { ma_NV = Convert.ToString(formData["ma_nhan_vien"]); }
                string ma_KH = "";
                if (formData.Keys.Contains("ma_khach_hang") && !string.IsNullOrEmpty(Convert.ToString(formData["ma_khach_hang"]))) { ma_KH = Convert.ToString(formData["ma_khach_hang"]); }
                long total = 0;
