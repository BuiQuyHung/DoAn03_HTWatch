    ...
    public class DonHangRepository : IDonHangRepository
    {
        ...
        public DonHangModel GetDatabyID(string MaDonHang)
        {
            ...
            try
            {
                ...
                var donHang = dt.ConvertTo<DonHangModel>().FirstOrDefault();

                if (donHang != null)
                {
                    // Fetch the order details data
                    var dtChiTiet = _dbHelper.ExecuteSProcedureReturnDataTable(out msgError, "sp_GetChiTietDonHangByMaDonHang",
                        "@MaDonHang", MaDonHang);
                    if (!string.IsNullOrEmpty(msgError))
                        throw new Exception(msgError);

                    // Convert and assign the list of order details
                    donHang.list_json_chitietdonhang = dtChiTiet.ConvertTo<ChiTietDonHangModel>().ToList();
                }
