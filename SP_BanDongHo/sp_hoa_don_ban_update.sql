...
CREATE PROCEDURE [dbo].[sp_hoa_don_ban_update]
(@MaHoaDonBan INT, 
...
AS
    BEGIN
		...
		BEGIN
		   ...
		   SELECT
			  JSON_VALUE(p.value, '$.maCTHDBan') as MaCTHDBan,
			  JSON_VALUE(p.value, '$.maHDBan') as MaHDBan,
		   ...
		   FROM OPENJSON(@list_json_chitiethoadonban) AS p;
			...
			-- Update data to table with STATUS = 2
			  ...
			  FROM #Results 
			  WHERE  ChiTietHDBan.MaCTHDBan = #Results.MaCTHDBan ;
		END;
    END;
GO